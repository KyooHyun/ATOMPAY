...
| 항목 | 내용 |
|------|------|
| 스택 | Spring Boot, Spring Data JPA, MySQL (개발 시 H2) |
| 핵심 주제 | 동시성 제어 · 멱등성 · 결제 상태 머신 |
| 검증 | JUnit 단위 테스트 + MySQL Testcontainers 동시성 테스트 |
...
| 상태 전이 | 허용된 전이만 성공하고 불법 전이는 예외로 차단됨 |
...
```
controller   # REST 엔드포인트, 요청/응답 변환
