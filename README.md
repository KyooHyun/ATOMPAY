...
| 항목 | 내용 |
|------|------|
| 스택 | Spring Boot 3.2 · Spring Data JPA · MySQL (개발 시 H2) · Java 21 |
...
취소·환불 경로는 `Authorization` 행에 비관적 락을 걸지만, `CardAccount`의 `availableAmount`를 복원할 때 락 없이 읽고 쓰는 문제가 있었습니다.
같은 카드의 서로 다른 두 승인이 동시에 취소되면, 두 스레드가 같은 잔액을 읽고 각자 +금액을 써서 한 쪽 복원이 유실됩니다.
`findByCardIdForUpdate`로 교체해 승인·취소·환불 모든 경로에서 CardAccount 락을 일관되게 적용했습니다.
...
- 외부 식별자(`authorizationId`)는 DB의 auto-increment PK를 그대로 노출하지 않고 UUID를 사용해, 다른 거래 ID 추측·열람(IDOR)을 방지합니다.
- 요청마다 `X-Request-Id` 헤더를 응답으로 반환합니다. 클라이언트가 헤더를 보내지 않으면 서버가 UUID를 생성합니다. 모든 로그 라인에 같은 ID가 찍혀 요청을 추적할 수 있습니다.
